...
> **Amended during execution.** The code below has three defects, found by running it against
> the real jslib sources. The committed implementation fixes them; treat the fixes as part of
> the task, not as scope creep.
>
> 1. `const taken: { [key: string]: boolean } = {}` in `transformClass` inherits `constructor`
>    and `toString` from `Object.prototype`, so `taken[key]` is truthy before any member is
>    seen and **every class constructor is silently dropped**. Use `Object.create(null)`.
> 2. `transformMember`'s switch has no accessor case, so `get`/`set` members vanish — taking
>    `Array.length` and `String.length` with them. See spec rule R4a.
> 3. Type aliases are never collected, so `String.replace`'s `string | FuncString` references
>    an undeclared name and the output does not type-check standalone. See spec rule R6a.
>
> Also: `ts.PropertyAccessExpression.name` is typed `Identifier | PrivateIdentifier` in
> TypeScript 3.9.10, so `assignedTypeNode` needs a `<ts.Identifier>` cast for
...
Run it from a directory with no `node_modules` ancestor — copy the file out first:

```bash
cp lib/JSLib.d.ts "$TMPDIR/JSLib.check.d.ts" && cd "$TMPDIR" \
  && npx tsc --noEmit --noLib --target es5 JSLib.check.d.ts
```

Expected: no errors.

Running it in place from the repository root does **not** work: TypeScript's default `typeRoots`
pulls in `node_modules/@types/{chai,fs-extra,node}`, which cannot compile under `--noLib` and
buries any real finding in unrelated errors.
